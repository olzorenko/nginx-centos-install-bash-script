...
#Fixed memory allocation limits on OpenVZ's (if on OpenVZ)
...
# Install APC if not installing on OpenVZ              
if [ ! -e "/proc/user_beancounters" ]
then
   #Install APC if you are not using openvz
  yum -y install php-pecl-apc --enablerepo=webtatic
fi

if [ `cat /proc/user_beancounters | grep " privvmpages " | awk '{print $4}'` > '49000' ]
then
   #Install APC on OpenVZ with 192mb or more privvmpages 
  yum -y install php-pecl-apc --enablerepo=webtatic
fi
